...
dragControls(renderer.domElement,arrow_group.children[0].children[0], arrow_group.children[0].children[1], img_group, img_index, img_length)
...
document.querySelectorAll('.direction-class').forEach(item => {
    item.addEventListener('click', function(){
      ...
      directionAnimation(index)}

    )
  })

function directionAnimation(index){

    //animation
    // createjs.Tween.get(img_group.children[0].material, { loop: true }).to({ opacity: 1 }, 500).wait(1500).to({ opacity: 0 }, 500);
    createjs.Tween
        .get(group.rotation, { loop: false })
        .to({ y: Math.PI *index * 0.25 }, 1500, createjs.Ease.getPowInOut(3))
        .to({ y: 0}, 1500, createjs.Ease.getPowInOut(3));
...
// backward(scene, img_group, arrow_group);
